...
const addCartToBackend = createAsyncThunk(
  ...
  async ({ phoneNumber, orderId, quantity }, thunkAPI) => {
    try {
      ...
      const response = await axios.post('https://shoppingplusbackend-production.up.railway.app/api/cart/add', {
        phoneNumber,
        orderId,
        quantity
      });
...
const getCartFromBackend = createAsyncThunk(
  ...
  async ({ phoneNumber }, thunkAPI) => {
    try {
      const response = await axios.get(`https://shoppingplusbackend-production.up.railway.app/api/cart/get/${phoneNumber}`);
